...
int count=1; int indexHis=0; int depth=0;
...
int main(int argc, char **argv)
{
	...
	/* The Shell runs in an infinite loop, processing input. */
		...
		fprintf(stdout, "Shell(pid=%d)%d> ", shell_pid,count);
		fflush(stdout);
		...
		while(buffer[0]=='!')
		{
		    ...
		    int t=buffer[1]-48-1;
			if(t<0||t>9||strlen(buffer)!=3||t>=indexHis)
				{
					fprintf(stderr,"Not vaild\n");
					break;
				}
			else
				{
					strcpy(buffer,temp[t]); 
					strcpy(temp[count],temp[t]);
				 }   

		}
		  
		strcpy(temp[indexHis++],buffer);
